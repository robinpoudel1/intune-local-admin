...
# Exit 0  = Compliant   (user is not a local admin)
# Exit 1  = Non-Compliant (user is still a local admin — triggers remediation)
#
...
try {
    ...
    if ($isAdmin) {
        Write-Output "Non-Compliant - $samAccount is a local admin and should be removed"
        exit 1
    } else {
        Write-Output "Compliant - $samAccount is not a local admin"
        exit 0
    }
} catch {
